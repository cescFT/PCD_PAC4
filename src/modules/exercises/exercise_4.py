...
def exercise_4(merged_df: pd.DataFrame):
    ...
    save_statistics(json_data)

    print("Fitxer d'estadístiques desat correctament.")


def save_statistics(json_data: dict):
    os.makedirs("src/report", exist_ok=True)
    with open("src/report/analisi_estadistic.json", "w") as json_file:
...
def calculate_ranking_by_branch(merged_df: pd.DataFrame):
    """
...
def calculate_statistics_per_branch(merged_df: pd.DataFrame):
    ...
    for study_type_branch in study_type_branches:
        ...
        values_abandonment = branch_by_year_abandonment['% Abandonament a primer curs'].tolist()
        ...
        branch_by_year_performance = branch_data.groupby('Curs Acadèmic').agg({
            'Taxa rendiment': 'mean'
        }).reset_index()

        years = branch_by_year_performance['Curs Acadèmic'].tolist()
        values_performance = branch_by_year_performance['Taxa rendiment'].tolist()
        slope, intercept, r_value, p_value, std_err = linregress(
            range(len(years)),
            values_performance
        )

        tendency_performance = "estable"
        if slope < -0.01:
            tendency_performance = "decreixent"
        elif slope > 0.01:
            tendency_performance = "creixent"
